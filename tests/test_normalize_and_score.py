...
def test_normalize(raw, expected):
    assert job._legacy.normalize(raw) == expected


def test_score_exact_match():
    assert job._legacy.score("foobarconsulting", "foobarconsulting") == 1.0


def test_score_both_empty_is_exact_match():
    assert job._legacy.score("", "") == 1.0


def test_score_containment_short_in_long():
    result = job._legacy.score("foobar", "foobarconsulting")
...
def test_score_containment_long_contains_short_either_order():
    a = job._legacy.score("foobar", "foobarconsulting")
    b = job._legacy.score("foobarconsulting", "foobar")
    assert a == b
...
def test_score_one_side_empty_skips_containment_branch():
    ...
    assert job._legacy.score("", "abc") == 0.0
...
def test_score_no_containment_uses_plain_ratio():
    ...
    assert job._legacy.score(a, b) == difflib.SequenceMatcher(None, a, b).ratio()
