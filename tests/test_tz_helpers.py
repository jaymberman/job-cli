...
def test_resolve_tz_token_none_returns_none_pair():
    ...
    # test_config.py.
    assert job.interview.resolve_tz_token(None) == (None, None)
...
def test_resolve_tz_token_aliases(token, expected):
    assert job.interview.resolve_tz_token(token) == expected


def test_resolve_tz_token_unknown_returns_none_pair():
    assert job.interview.resolve_tz_token("XX") == (None, None)


def test_unknown_tz_message_names_the_bad_token_and_supported_list():
    msg = job.interview.unknown_tz_message("XX")
    assert "XX" in msg
