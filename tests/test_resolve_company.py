...
def test_exact_normalized_match_short_circuits_without_scoring():
    ...
    assert job.company.resolve_company_key(pool, "foobar consulting") == "foobarconsulting"


def test_exact_match_case_and_spacing_insensitive():
    pool = {"1": rec("foobar Consulting")}
    assert job.company.resolve_company_key(pool, "foobarconsulting") == "foobarconsulting"


def test_empty_pool_returns_none():
    assert job.company.resolve_company_key({}, "anything") is None
...
def test_data_guard_prevents_fuzzy_reinterpretation_across_pools():
    ...
    pool = job.company.active_records(data)

    assert job.company.resolve_company_key(pool, "Big Corp", data=data) is None


def test_auto_accepts_confident_single_candidate():
    pool = {"1": rec("foobar Consulting")}
    assert job.company.resolve_company_key(pool, "foobar") == "foobarconsulting"


def test_auto_accept_never_calls_confirm(monkeypatch):
    pool = {"1": rec("foobar Consulting")}
    monkeypatch.setattr(job.company, "confirm", lambda prompt: (_ for _ in ()).throw(
        AssertionError("confirm() should not be called on a confident auto-accept")))
    job.company.resolve_company_key(pool, "foobar")
...
def test_confirm_threshold_prompts_and_accepts(monkeypatch):
    ...
    result = job.company.resolve_company_key(pool, "davidoff")
...
def test_confirm_threshold_prompts_and_declines_returns_none(answer_input):
    pool = {"1": rec("Davita")}
    answer_input("n")
    assert job.company.resolve_company_key(pool, "davidoff") is None
...
def test_below_confirm_threshold_returns_none_without_prompting(monkeypatch):
    ...
    assert job.company.resolve_company_key(pool, "Qwxyz Industries") is None
...
def test_close_margin_between_top_two_falls_to_confirm(monkeypatch, answer_input):
    ...
    monkeypatch.setattr(job.company, "score", fake_score)
    answer_input("y")
    assert job.company.resolve_company_key(pool, "whatever") == "aaacorp"
...
def test_resolve_active_finds_record_id_for_matched_company():
    ...
    assert job.company.resolve_active(data, "big corp") == "11111111"


def test_resolve_active_ignores_soft_deleted_records():
    deleted = rec("Big Corp", id_="deaddead", deleted=True)
    data = {"deaddead": deleted}
    assert job.company.resolve_active(data, "big corp") is None


def test_resolve_active_no_match_returns_none():
    data = {"11111111": rec("Big Corp", id_="11111111")}
    assert job.company.resolve_active(data, "Totally Unrelated Industries") is None
...
def test_active_records_excludes_deleted():
    ...
    assert job.company.active_records(data) == {"11111111": active}
...
def test_deleted_records_only_deleted():
    ...
    deleted = rec("Deleted Co", id_="22222222", deleted=True)
    data = {"11111111": active, "22222222": deleted}
    assert job.company.deleted_records(data) == {"22222222": deleted}
