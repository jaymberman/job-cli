...
def test_scroll_stops_includes_column_boundaries_within_range():
    stops = job.scroll_stops([0, 10, 25, 40], total_width=60, term_width=20)
    assert stops == [0, 10, 25, 40]


def test_scroll_stops_clamps_columns_beyond_max_offset():
    stops = job.scroll_stops([0, 50, 55], total_width=60, term_width=20)
    assert stops == [0, 40]


def test_scroll_stops_single_zero_stop_when_table_fits():
    stops = job.scroll_stops([0, 5], total_width=20, term_width=30)
    assert stops == [0]
...
def test_read_key_regular_character(monkeypatch):
    monkeypatch.setattr(job.os, "read", lambda fd, n: b"q")
    assert job.read_key(3) == b"q"


def test_read_key_lone_escape_with_no_followup(monkeypatch):
    monkeypatch.setattr(job.os, "read", lambda fd, n: b"\x1b")
    monkeypatch.setattr(job.select, "select", lambda *a, **kw: ([], [], []))
    assert job.read_key(3) == b"\x1b"


def test_read_key_arrow_sequence(monkeypatch):
    reads = iter([b"\x1b", b"[", b"C"])
    monkeypatch.setattr(job.os, "read", lambda fd, n: next(reads))
    monkeypatch.setattr(job.select, "select", lambda *a, **kw: ([3], [], []))
    assert job.read_key(3) == b"\x1b[C"
