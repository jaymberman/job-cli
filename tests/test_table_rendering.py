...
def test_build_table_lines_basic_structure():
    lines, column_starts, row_colors = job._legacy.build_table_lines([make_rec("Big Corp")])
...
def test_build_table_lines_note_column_shows_value():
    lines, _, _ = job._legacy.build_table_lines([make_rec("Big Corp", note="Referred by a friend")])
    assert "Referred by a friend" in lines[2]


def test_build_table_lines_note_column_is_last_before_deleted():
    lines, _, _ = job._legacy.build_table_lines([make_rec("Big Corp", deleted=True, deleted_at="2026-02-01")],
                                         show_deleted=True)
    header = lines[0]
...
def test_build_table_lines_show_deleted_adds_column():
    ...
    lines, _, _ = job._legacy.build_table_lines([r], show_deleted=True)
    assert "Deleted" in lines[0]
    assert "2026-02-01" in lines[2]


def test_build_table_lines_unset_interview_renders_em_dash():
    lines, _, _ = job._legacy.build_table_lines([make_rec("Big Corp")])
    assert "—" in lines[2]
...
def test_row_color_none_when_not_deleted_and_no_interview():
    _, _, row_colors = job._legacy.build_table_lines([make_rec("Big Corp")])
...
def test_classify_interview_color_none_when_unset():
    assert job._legacy.classify_interview_color(make_rec("Big Corp")) is None
...
def test_classify_interview_color_today_within_30_min_window(freeze_now, freeze_date):
    ...
    assert job._legacy.classify_interview_color(r) == "today"
...
def test_classify_interview_color_past_after_30_min_cutoff(freeze_now, freeze_date):
    ...
    assert job._legacy.classify_interview_color(r) == "past"
...
def test_classify_interview_color_future_date(freeze_now, freeze_date):
    ...
    assert job._legacy.classify_interview_color(r) == "future"


# ---- colorize ---------------------------------------------------------------

def test_colorize_passthrough_when_no_color_key():
    assert job._legacy.colorize("hello", None) == "hello"


def test_colorize_wraps_with_forced_text_and_highlight_codes():
    result = job._legacy.colorize("hello", "today")
    expected = f"\x1b[{job._legacy.FORCED_TEXT_COLOR};{job._legacy.ROW_HIGHLIGHTS['today']}mhello\x1b[0m"
...
def test_print_table_plain_when_it_fits(term_size, tty, capsys):
    ...
    job._legacy.print_table([make_rec("Big Corp")])
...
def test_print_table_applies_color_when_stdout_is_tty(term_size, tty, capsys):
    ...
    job._legacy.print_table([r])
    out = capsys.readouterr().out
    assert f"\x1b[{job._legacy.FORCED_TEXT_COLOR};{job._legacy.ROW_HIGHLIGHTS['deleted']}m" in out
...
def test_print_table_dispatches_to_interactive_scroll_when_too_wide(term_size, tty, monkeypatch):
    ...
    monkeypatch.setattr(job._legacy, "scroll_table_interactive", fake_scroll)
    job._legacy.print_table([make_rec("Big Corp")])
...
def test_print_table_falls_back_to_plain_when_too_wide_but_not_both_ttys(term_size, tty, capsys, monkeypatch):
    ...
    monkeypatch.setattr(job._legacy, "scroll_table_interactive", explode)
    job._legacy.print_table([make_rec("Big Corp")])
    out = capsys.readouterr().out
    assert "Big Corp" in out
